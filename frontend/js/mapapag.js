...
const statusEl = (() => { const id = "mapa-status"; const existing = qs(id); if(existing) return existing; const s = document.createElement("div"); s.id = id; s.style.margin="10px 0"; roteiroContainerEl.parentNode.insertBefore(s, roteiroContainerEl); return s; })();
...
function renderizarRoteiroBanco(pontos){
  ...
  pontos.forEach(p=>{
    const card=document.createElement("div"); card.className="ponto-card"; card.style.border="1px solid #ccc"; card.style.padding="10px"; card.style.marginBottom="8px"; card.style.borderRadius="6px";
    card.innerHTML=`<strong>${escapeHtml(p.nome)}</strong><br><span style="color:#555">(${formatCoord(p.latitude)}, ${formatCoord(p.longitude)})</span><div style="margin-top:6px">${escapeHtml(p.descricao||"-")}</div>`;
...
async function main(){
  try{
    ...
    if(clima?.current){
      climaEl.innerHTML=`<h4>Clima</h4><div>${clima.current.temp_c}°C — ${escapeHtml(clima.current.condition?.text)}</div>`;
    }
    ...
    if(pontosExistentes.length>0){
      ...
      plotarPontos(pontosExistentes); // <<< PLOTAR NO MAPA
      return;
    }
    ...
    const atividades = [];
    roteiroIA.dias.forEach(d=>{ d.atividades.forEach(a=>{ atividades.push({titulo:a.titulo, descricao:(a.descricao||"").substring(0,5000), lat:a.coordenadas?.lat??null, lon:a.coordenadas?.lon??null}); }); });
    ...
    plotarPontos(pontosSalvos); // <<< PLOTAR NO MAPA
